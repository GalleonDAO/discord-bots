...
} = require("./utils/discordInteractionHandler");
const {
  KNOWN_LABELS,
  LOG_SEVERITY,
} = require("./services/azureLoggingService");
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (!command) return;
  console.log(`user is admin? ${isAdmin(interaction.member.roles.cache)}`);
  const interactionHandler = isAdmin(interaction.member.roles.cache)
    ? new AdminInteractionHandler(interaction, logger)
    : new DiscordInteractionHandler(interaction, logger);
  try {
    logger.logCounter(KNOWN_LABELS.COMMAND_USED, {
      ...
      [interaction.options.data[0]
        ...
        : "params"]: interaction.options.data[0]
    ...
    await interactionHandler.reply({
...
const isAdmin = (roles) => {
  return (
    roles.some((role) => Object.values(ADMIN_ROLES).includes(role.id)) ||
    roles.some((role) => Object.values(DEBUG_ADMIN_ROLES).includes(role.id))
  );
};
