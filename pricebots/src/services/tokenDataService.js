...
module.exports.fetchCoingeckoData = async (tokenId) => {
  try {
    const tokenData = await (await fetch(`${COINGECKO_URL}/${tokenId}`)).json()
...
module.exports.fetchTokensetsData = async (tokenId) => {
  try {
    const tokenData = await (await fetch(`${TOKENSETS_URL}/${tokenId}`)).json()
...
module.exports.fetchTokensetsChange = async (tokenId) => {
  try {
    const tokenData = await (
      await fetch(
        `${TOKENSETS_HISTORY_URL}/${tokenId}?interval=${INTERVAL}&currency=usd`,
      )
    ).json()
